  ...
  */
  var normRand = function (m, s) {
    ...
    if(0.5 - Math.random() > 0) {
      if (c * Math.sin(Math.PI * 2 * b) * s + m > 0) {
        return Math.floor(c * Math.sin(Math.PI * 2 * b) * s + m);
      } else {
        return -Math.floor(c * Math.sin(Math.PI * 2 * b) * s + m);
      }
    }else{
      if (c * Math.cos(Math.PI * 2 * b) * s + m > 0) {
  ...
  p.checkForRecovered = () => {
    ballsInfectionTime.forEach(ball => {
      if (ball.time !== undefined &&
        (Date.now() - ball.time > normRand(recoveryTimeInMillis, 900))) {
        ball.time = undefined;
  ...
  p.reset = function (args) {
    ...
    quarantined = (args && args.quarantined) ? args.quarantined : defaultValues.quarantined;
